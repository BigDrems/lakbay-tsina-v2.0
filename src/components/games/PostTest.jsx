...
const PostTest = ({ onComplete }) => {
  ...
  const handleComplete = () => {
    ...
    const finalScore = Math.round(
      (correctAnswers / POSTTEST_DATA.questions.length) * 100
    );
    setScore(finalScore);
                  ...
                  <div>
                    <span
                      className={`font-semibold ${
                        entry.score >= 30 ? "text-green-600" : "text-red-600"
                      }`}
                    >
                      {entry.score}%
                    </span>
                    <span className="text-gray-500 ml-2">
                      ({entry.answered}/{entry.questions})
                    </span>
                  </div>
        ...
        <div className="bg-white rounded-lg shadow-lg p-3 sm:p-6">
          <div className="text-center mb-6">
            <h1 className="text-xl sm:text-3xl font-bold text-[#6B3100] mb-2">
              🎯 Post-test sa Sinaunang Tsina
            </h1>
            <p className="text-sm sm:text-base text-gray-600 mb-2">
              Ipakita ang inyong natutunang kaalaman tungkol sa sinaunang
              kabihasnang Tsina!
            </p>
